...
function TrabalhosPage() {
  ...
  const [f, setF] = useState({
    ...
    orientador_cargo: "",
    tema: "",
    nivel_academico: "Médio" as "Médio" | "Superior",
    paginas: 8,
    formato_citacao: "APA" as "APA" | "ABNT" | "Vancouver",
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!TIPOS_OK.includes(file.type) && !/\.(pdf|docx?|xlsx?)$/i.test(file.name)) {
          toast.error(`${file.name}: só PDF, Word ou Excel`);
          continue;
        }
  ...
  const removerAnexo = (path: string) => setAnexos((p) => p.filter((a) => a.path !== path));
  ...
  const submeter = async () => {
    ...
    if (tipoFonte === "anexo" && anexos.length === 0) {
      toast.error("Anexa pelo menos um ficheiro.");
      return;
    }
    setGerando(true);
    try {
      const storedTelefone = localStorage.getItem("telefone") || "";
      await create({ data: { dados: f, tipo_fonte: tipoFonte, anexos, telefone: storedTelefone } });
      ...
      <div className="mx-auto max-w-4xl px-4 py-6 md:px-8">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Preenche o formulário. O crédito (1 trabalho) só é descontado quando clicas em{" "}
            <strong>Gerar trabalho</strong>.
          </p>
          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-sm">
            ...
            {disponiveis === 0 && (
              <Link to="/app/planos" className="ml-2 text-primary underline">
                Comprar
              </Link>
            )}
          </div>
        ...
        <section className="space-y-6 rounded-2xl border border-border bg-card p-5 md:p-6">
          <Bloco titulo="Dados do aluno">
            ...
            <Campo label="Turma *">
              <Input value={f.turma} onChange={(e) => upd("turma", e.target.value)} />
            </Campo>
            <Campo label="Número do aluno *">
              <Input value={f.numero_aluno} onChange={(e) => upd("numero_aluno", e.target.value)} />
            </Campo>
          </Bloco>

          <Bloco titulo="Instituição">
            <Campo label="Nome da escola/instituição *">
              <Input value={f.escola} onChange={(e) => upd("escola", e.target.value)} />
            </Campo>
            <Campo label="Curso ou disciplina *">
              <Input value={f.curso} onChange={(e) => upd("curso", e.target.value)} />
            </Campo>
          </Bloco>

          <Bloco titulo="Orientador">
            <Campo label="Nome do orientador *">
              <Input
                value={f.orientador_nome}
                onChange={(e) => upd("orientador_nome", e.target.value)}
              />
            </Campo>
            <Campo label="Cargo *">
              <Input
                value={f.orientador_cargo}
                onChange={(e) => upd("orientador_cargo", e.target.value)}
              />
            </Campo>
          </Bloco>
          ...
          <Bloco titulo="Trabalho">
            ...
            <Campo label="Nível académico *">
              <Select
                ...
                onValueChange={(v) => upd("nivel_academico", v as any)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <Campo
              label="Formato de citação (referências bibliográficas) *"
              className="md:col-span-2"
            >
              <Select
                value={f.formato_citacao}
                onValueChange={(v) => upd("formato_citacao", v as any)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Define como serão apresentadas as fontes/referências no fim do trabalho.
              </p>
            </Campo>
          </Bloco>
          ...
          <Bloco titulo="Data">
            ...
            <Campo label="Ano *">
              <Input
                ...
                min={2024}
                value={f.ano}
                onChange={(e) => upd("ano", Number(e.target.value) || ANO_ACTUAL)}
              />
            </Campo>
            <Campo label="Cidade *">
            ...
            <div className="md:col-span-3 flex flex-col gap-3 sm:flex-row">
              ...
              >
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  A IA pesquisa e gera automaticamente.
                </div>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  PDF / Word / Excel. Feito manualmente — até 6h.
                </div>
              </button>
            </div>
            ...
            {tipoFonte === "anexo" && (
              <div className="md:col-span-3 space-y-3">
                <div className="flex items-start gap-2 rounded-lg bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-300">
                  ...
                  <p>
                    Como o material é anexado, o trabalho será feito manualmente pelo administrador
                    e entregue em até <strong>6 horas</strong>.
                  </p>
                </div>
                <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-muted/30 p-6 text-center hover:bg-muted">
                  ...
                  <span className="text-sm font-medium">
                    {uploading ? "A enviar…" : "Clica para anexar (PDF, Word, Excel)"}
                  </span>
          ...
          <div className="flex justify-end pt-2">
            <Button size="lg" onClick={submeter} disabled={gerando || disponiveis <= 0}>
              ...
              {tipoFonte === "internet"
                ? "Gerar trabalho (−1 crédito)"
                : "Enviar ao ADM (−1 crédito)"}
            </Button>
          </div>
        </section>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="truncate font-medium">
                        {t.titulo ?? t.dados_formulario?.tema}
                      </div>
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                        <Badge variant="secondary">
                          {t.tipo_fonte === "anexo" ? "📎 Anexo" : "🌐 Internet"}
                        </Badge>
