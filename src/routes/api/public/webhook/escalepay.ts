    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const amount: number = Number(body.amount ?? body.valor ?? body.value ?? body.price ?? 0);
          ...
          if (novoPlano) {
            ...
            update.plano_expira = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
          }
          ...
          await supabaseAdmin.from("webhook_logs").update({ processed: true }).eq("id", log!.id);

          return new Response(JSON.stringify({ ok: true, reference, user_id: profile.id }), {
            status: 200,
            headers: { "content-type": "application/json" },
          });
        } catch (err) {
      ...
      GET: async () =>
        new Response(JSON.stringify({ ok: true, endpoint: "escalepay-webhook" }), {
          status: 200,
          headers: { "content-type": "application/json" },
        }),
    },
