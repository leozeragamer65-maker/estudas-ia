...
function LoginPage() {
  ...
  const entrar = async (e: FormEvent) => {
    ...
    const cleanPhone = id.includes("@") ? "" : normalizarTelefone(id);
    if (cleanPhone) {
      localStorage.setItem("telefone", cleanPhone);
    } else {
      try {
        const res = await lookup({ data: { email: id.toLowerCase() } });
        if (res?.telefone) {
          localStorage.setItem("telefone", normalizarTelefone(res.telefone));
        }
      } catch (e) {
        console.error("Erro ao obter telefone por email:", e);
      }
    }
    navigate({ to: "/app" });
  };
  ...
  const registar = async (e: FormEvent) => {
    ...
    localStorage.setItem("telefone", normalizarTelefone(telefone));
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          <Button type="button" variant="outline" disabled className="relative w-full opacity-80">
