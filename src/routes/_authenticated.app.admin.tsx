...
function AdminPage() {
  ...
  const { data: prof, isLoading: lp } = useQuery({
      ...
      <Centro>
        <div className="max-w-sm text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Esta área é apenas para o administrador.
          </p>
        </div>
      </Centro>
    );
        ...
        <Tabs defaultValue="trabalhos" className="mt-6">
          ...
          <TabsContent value="trabalhos" className="mt-4">
            <p className="text-sm text-muted-foreground">
              Baixa os materiais do aluno, faz o trabalho e envia o ficheiro final.
            </p>
            {isLoading ? (
              <div className="mt-4">
                  ...
                  <CardTrabalho
                    ...
                    onEntregar={async (file) => {
                      const { path, token } = await sign({
                        data: { trabalhoId: t.id, nome: file.name },
                      });
                ...
                <div key={a.id} className="rounded-xl border border-border bg-card p-4">
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`h-4 w-4 ${i < a.estrelas ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`}
                      />
                    ))}
...
function CardTrabalho({
  ...
  t: any;
  onBaixar: (p: string) => void;
  onEntregar: (f: File) => Promise<void>;
}) {
  ...
  return (
    <li className="rounded-xl border border-border bg-card p-4">
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
        <Linha
          k="Aluno"
          v={`${d.nome_completo ?? "—"} (${d.turma ?? "—"}, nº ${d.numero_aluno ?? "—"})`}
        />
          ...
          <ul className="space-y-1">
            {t.anexos.map((a: any) => (
              ...
              >
                ...
                <Button size="sm" variant="ghost" onClick={() => onBaixar(a.path)}>
