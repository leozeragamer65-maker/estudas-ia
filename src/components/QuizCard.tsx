...
export function QuizCard({ quiz }: { quiz: Quiz }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <div className="mt-3 grid gap-2">
        {quiz.opcoes.map((op, i) => {
          ...
          return (
            ...
            >
              <span className="mr-2 font-display text-sm text-muted-foreground">
      ...
      {resultado && (
        <div
          className={`mt-4 rounded-lg p-3 text-sm ${resultado.correta ? "bg-emerald-500/10" : "bg-destructive/10"}`}
        >
          <div className="font-medium">
            {resultado.correta ? "✅ Resposta correta!" : "❌ Resposta errada."}
          </div>
          {resultado.explicacao && (
            <div className="mt-1 text-muted-foreground">{resultado.explicacao}</div>
          )}
        </div>
      )}
    </div>
